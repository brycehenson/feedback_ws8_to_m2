classdef (Sealed) WLM_shared_pool < handle

%{
    Singleton class for a WS6 or WS7 High Finesse wavemeter object. It allows for the readout of
    the WLM_shared_pool in either nm or MHz, regulates the exposure time and if
...
%}
    ...
    properties
        ...
        timeout = 5; % timeout for wavelength or frequency measurement.
    end
    
    properties (Access = private)
        ar % Struct containing the argument name strings from WLM_shared_pool header
        active_channel = -1 % Number of channel that is read out by wavemeter
        num_channels = 4 % Number of input channels of wavemeter
    end
    
    % Regulate that only 1 instance of WLM_shared_pool can exist with a class-wide,
    % static method
    methods (Static)
        % The "effective" constructor, called as WLM_shared_pool.getInstance
        function wlm = getInstance()
            persistent WLMRef
            if isempty(WLMRef) || ~isvalid(WLMRef)
                WLMRef = WLM_shared_pool();
            end
            wlm = WLMRef;
        end
    end
...
%                 setpt_mon(labindex)
            ...
            % Instantiate the WaitForWLMEvent function (see WLM_shared_pool manual)
        ...
        function [WL, freq] = ReadWLM(obj, what, channel)
            ...
            while (~strcmp(what, 'WLM_shared_pool')  && isnan(freq)) || ...
                ...
                if strcmp(what, 'WLM_shared_pool') || strcmp(what, 'both')
    ...
    methods (Access = public)
        ...
        function WL = GetWL(obj,channel)
            [WL, ~] = ReadWLM(obj, 'WLM_shared_pool',channel); % Output in nm
        end
