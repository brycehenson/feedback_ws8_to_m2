...
elseif method_key==2 %feedback loop!
    ...
    while true %error catching loop
        ...
        ecd_relock_pd_thresh=0.58;%0.65 %min power at ecd pd before relock attempt
        ...
        %pid system for the etalon feddback
        etl_feedback_timeout=10; %how long to try the etalon feedback
        etl_lock_thresh=2000;%MHZ tolerance about half the step size
        pid_etalon=[];
        pid_etalon.k_int=5e-4;
        pid_etalon.k_prop=3e-5;
        pid_etalon.int_lim=200;
        pid_etalon.outlims=[1,99];
        pid_etalon.slew_lim=8;
        pid_etalon.dout_lim=10;
        ...
        try
            while true %feedback loop
                ...
                try %Check for new set point
                    if labProbe
                        ...
                        if data ~=pid_res.setpt
                            ...
                            old_setpt=pid_res.setpt;
                            pid_res.setpt = data; %get set point from server
                            if abs( pid_res.setpt-old_setpt)>align_setpoint_change
                                realign_now=true;
                            end
                        else
                ...
                if pid_res.aw<1e-2  || abs(pid_res.error)>resonator_range
                        ...
                        time_etl_feedback_start=posixtime(datetime('now'));
                        ...
                        while feedback_etalon
                            ...
                            if abs(pid_etalon.error)<etl_lock_thresh 
                                ...
                                fprintf('sucess')
                            elseif pid_etalon.aw<1e-1 || ...
                                posixtime(datetime('now'))>time_etl_feedback_start+etl_feedback_timeout
                ...
                if time_last_res_check+intv_res_check<pid_res.time &&...
                    ...
                    res_lock_attempts=0;
                    fprintf('lock check last align %f s \n',pid_res.time-time_last_beam_algin)
                    if pid_res.time-time_last_beam_algin<wait_for_align_before_lock
                        %shedule relock for when the beam alingment is done
                        fprintf('waiting for align before lock\n')
                        time_last_res_check=pid_res.time-intv_res_check+wait_for_align_before_lock; %change the last check time
                    else
