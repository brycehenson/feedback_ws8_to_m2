...
if isempty(solstis)
    ...
    % Open communication
    solstis = tcpip('150.203.178.241', 39933,... % Enter the ICE BLOC ip address and port number here.
...
%     tic
    ret = query(solstis,['{"message":{"transmission_id":[1],"op":"start_link",' ...
        '"parameters":{"ip_address":"150.203.178.241"}}}']); % Enter the control computer's IP address here.
%     toc
    warning('on',id)    
...
elseif isequal(solstis.status,'closed')
    fopen(solstis)
end
